...
- 댓글 내용의 비속어 예측 정확도를 향상시키기 위해, 간결한 불용어(의미를 전달하지 않는 단어들을 가리키는 용어) 목록을 사용하여 자연어 처리를 수행합니다.  
...
- 불용어를 제거하였음에도 동일한 점수가 나오는 것을 확인하고 전처리의 문제가 아니라고 판단했습니다.
...
- **신고 처리과정 개선**: 현재 신고 관리 페이지가 없기 때문에 사용자가 신고할 때 자동으로 처리가 이루어지도록 구성했습니다.
  차후 신고 관리 페이지를 추가하고 신고된 댓글을 관리자 페이지에서 관리자가 검토하여 처리할 수 있도록 개선하겠습니다.  
